...
namespace App\Rails\Domain\Entity;

use php7extension\core\exceptions\InvalidCallException;
use php7extension\core\exceptions\UnknownPropertyException;
use php7extension\core\helpers\ClassHelper;
use php7extension\core\helpers\EntityType;
use php7extension\core\helpers\Helper;
use php7extension\yii\helpers\ArrayHelper;
use php7rails\domain\interfaces\ValueObjectInterface;
use php7rails\domain\traits\entity\ValidatorTrait;
use php7extension\core\helpers\ReflectionHelper;
use ReflectionClass;
...
class BaseEntity
{
    ...
    public function __set($name, $value) {
        ...
        if(method_exists($this, $setter)) {
            ...
            $this->attachBehavior($name, $value instanceof Behavior ? $value : ClassHelper::createObject($value));
        ...
        if(method_exists($this, 'get' . $setter)) {
            throw new InvalidCallException('Setting read-only property: ' . get_class($this) . '::' . $setter);
        }
    ...
    public function getAttribute($name, $inRaw = false) {
        ...
        if(method_exists($this, 'set' . $getter)) {
            throw new InvalidCallException('Getting write-only property: ' . get_class($this) . '::' . $getter);
        }
